        ...
        <div className="mt-3 flex w-full min-w-0 flex-row items-stretch gap-2 overflow-hidden sm:mt-4">
          {(
            ...
            <Button
              ...
              disabled={revealing}
              aria-busy={revealing}
              className="h-10 min-w-0 flex-1 basis-0 px-3 text-xs transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] sm:text-sm bg-[#25D366] hover:bg-[#1da851] text-white border-0"
              asChild={Boolean(revealedWhatsapp)}
              onClick={revealedWhatsapp ? undefined : handleRevealWhatsapp}
              data-wa-target-type="provider"
              data-wa-target-id={provider.id}
            >
              <a
                href={revealedWhatsapp ? whatsappLink(revealedWhatsapp, buildSmartMessage(displayName, provider.category, geoCity, geoState)) : undefined}
                target="_blank"
                rel="noopener noreferrer"
                data-wa-target-type="provider"
                data-wa-target-id={provider.id}
                data-wa-target-label={displayName}
                onClick={() => revealedWhatsapp && trackWhatsAppClick(provider.id, provider.slug, trackingSource)}
              ...
              >
                ...
                <span className="truncate">{revealing ? 'Abrindo...' : 'WhatsApp'}</span>
              </a>
            </Button>
          )}
