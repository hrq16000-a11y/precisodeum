            ...
            >
              <div className="flex min-w-0 items-start justify-between gap-2 overflow-hidden">
                ...
                {engTier.tier === 'ouro' && (provider.activitySignal || isOnline) && (
                  <span
                    title="Padrão Ouro: nível Ouro+ e ativo nos últimos 7 dias — recebe boost de visibilidade no ranking."
                    aria-label="Padrão Ouro: nível Ouro+ e atividade recente"
        ...
        <div className="mt-3 flex w-full min-w-0 flex-row items-stretch gap-2 overflow-hidden sm:mt-4">
          {!contactUnavailable && (
            <Button
              size="sm"
              aria-busy={revealing}
              data-cta={revealedPhone ? 'phone' : 'whatsapp'}
              className={`h-10 min-w-0 flex-1 basis-0 px-3 text-xs transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] sm:text-sm border-0 text-white ${revealedPhone ? 'bg-primary hover:bg-primary/90' : 'bg-[#25D366] hover:bg-[#1da851]'}`}
              asChild
            >
              <a
                href={
                  revealedPhone
                    ? `tel:${revealedPhone.replace(/[^\d+]/g, '')}`
                    : revealedWhatsapp
                      ? whatsappLink(revealedWhatsapp, buildSmartMessage(displayName, provider.category, geoCity, geoState))
                      : '#'
                }
                target={revealedPhone ? undefined : '_blank'}
              ...
              >
                {revealedPhone ? (
                  <Phone className="h-4 w-4 shrink-0 icon-cta" aria-hidden="true" />
                ) : (
                ...
                <span className="truncate">{revealing ? 'Abrindo...' : revealedPhone ? 'Ligar' : 'WhatsApp'}</span>
              </a>
            </Button>
          )}
