...
const ProviderCard = ({ provider, isFallback = false, trackingSource = 'home', index = 0 }: ProviderCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-5 relative min-w-0">
          ...
          <div className="min-w-0 flex-1">
            ...
            >
              <div className="flex items-start justify-between gap-2 min-w-0">
                <h3 className="truncate font-display text-sm sm:text-base font-bold text-foreground group-hover:text-accent transition-colors min-w-0 flex-1">
                  {displayName}
                </h3>
                {engTier.showCrown && (
        ...
        <div className="mt-3 sm:mt-4 flex flex-row gap-2 w-full">
          {provider.whatsapp && (
            <Button size="sm" className="flex-1 min-w-0 h-9 px-2 text-xs sm:text-sm transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] bg-[#25D366] hover:bg-[#1da851] text-white border-0" asChild>
              <a
          ...
          <Button variant="outline" size="sm" className={`h-9 px-3 text-xs sm:text-sm shrink-0 whitespace-nowrap transition-transform duration-200 hover:scale-[1.02] active:scale-[0.98] ${provider.whatsapp ? 'min-w-fit' : 'flex-1'}`} asChild>
            <Link
              to={`/profissional/${provider.slug}`}
              onClick={() => trackProfileClick(provider.id, provider.slug, trackingSource)}
              {...handlers}
            ...
            >
              Ver Perfil <ArrowRight className="ml-1 h-3.5 w-3.5 shrink-0" />
            </Link>
          </Button>
        </div>
