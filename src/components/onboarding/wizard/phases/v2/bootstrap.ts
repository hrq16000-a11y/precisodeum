...
export function buildOnboardingV2BootstrapState({ profile, provider }: BootstrapInput): Partial<OnboardingState> | null {
  ...
  return {
    ...
    profile: {
      ...
      facebook_url,
      website_url,
      street: String(provider?.street || '').trim() || undefined,
      street_number: String(provider?.street_number || '').trim() || undefined,
      complement: String(provider?.complement || '').trim() || undefined,
      postal_code: String(provider?.postal_code || '').trim() || undefined,
      show_full_address: provider?.show_full_address === true,
      street_suggested: String(provider?.street || '').trim() || undefined,
      bairro_sugerido_cep: neighborhood || undefined,
