...
const TestimonialsSection = () => {
  ...
  const next = useCallback(() => goTo((page + 1) % totalPages), [page, totalPages, goTo]);
  const prev = useCallback(() => goTo((page - 1 + totalPages) % totalPages), [page, totalPages, goTo]);
  ...
  return (
    <section className="relative py-16 overflow-hidden">
      ...
      <div className="container relative">
        ...
        <div
          key={page}
          className="grid gap-5 md:grid-cols-3 [content-visibility:auto] [contain-intrinsic-size:1px_500px] animate-fade-in"
        >
          {visible.map((t, i) => {
