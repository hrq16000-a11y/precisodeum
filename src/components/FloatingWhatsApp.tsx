...
const FloatingWhatsApp = ({ jobTitle }: Props) => {
  const floatingEnabled = useSettingValue('whatsapp_floating_enabled');
  const supportPhone = useSettingValue('whatsapp_support_phone') || FALLBACK_PHONE;
  const customMessage = useSettingValue('whatsapp_floating_message');

  const message = jobTitle
    ? `Olá! Vi a vaga "${jobTitle}" no Preciso de um e gostaria de mais informações.`
    : customMessage || 'Olá! Preciso de ajuda no Preciso de um.';
  ...
  // Respect admin toggle — default to enabled if setting not yet set
  if (floatingEnabled === 'false' || shouldHide) return null;

  const hasBottomNav = !shouldHide;
  ...
  return (
    <motion.a
      ...
      className="fixed right-4 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg"
      style={{
        zIndex: 9999,
        bottom: hasBottomNav
