import type { TensorNode } from './tensorTypes';
...
function adapt(s: PriorLayerSnapshot): TensorNode {
  const id = s.id ?? `${s.layer}:0`;
  const stage = s.stage ?? 'STAGE_0_READ_ONLY';
  const pressure = s.pressure ?? 0; const curvature = s.curvature ?? 0; const density = s.density ?? 0;
  const neighbors = Object.freeze([...(s.neighbors ?? [])].sort());
  return Object.freeze({ id, layer: s.layer, stage, liveExecutionEnabled: s.liveExecutionEnabled ?? false, retryEnabled: s.retryEnabled ?? false, backgroundEnabled: s.backgroundEnabled ?? false, realUsersAllowed: s.realUsersAllowed ?? false, pressure, curvature, density, neighbors, signature: `${id}:${s.layer}:${stage}:${pressure}:${curvature}:${density}:${neighbors.join(',')}` });
}
...
export const CANONICAL_TENSOR_LAYERS: readonly string[] = Object.freeze(['equilibrium-mechanics','convergence-calculus','canonical-algebra','governance-mesh','immutable-core','enforcement','isolation','integrity','stability','causality','replay','history','recorder','certification','atomic-governance','atomic-promotion','atomic-simulation']);
export function adaptAllTensorInputs(snapshots: readonly PriorLayerSnapshot[]): readonly TensorNode[] { return Object.freeze(snapshots.map(adapt)); }
export function buildDefaultTensorInputs(): readonly TensorNode[] { const layers = CANONICAL_TENSOR_LAYERS; return Object.freeze(layers.map((layer, i) => { const next = layers[(i + 1) % layers.length]; return adapt({ layer, neighbors: Object.freeze([`${next}:0`]) }); })); }
