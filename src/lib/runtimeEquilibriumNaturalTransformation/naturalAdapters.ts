import type { NaturalComponent } from './naturalTransformationTypes';

export interface PriorNaturalSnapshot {
  readonly id?: string;
...
function adapt(s: PriorNaturalSnapshot): NaturalComponent {
  ...
  const commutativity = s.commutativity ?? 1;
  ...
  return Object.freeze({
    ...
    naturality, identity, determinism, stability, commutativity, morphisms,
    signature: `${id}:${s.layer}:${stage}:${naturality}:${identity}:${determinism}:${stability}:${commutativity}:${morphisms.join(',')}`,
  });
}

const make = (layer: string) => (s?: Partial<PriorNaturalSnapshot>) => adapt({ ...(s ?? {}), layer });

export const CANONICAL_NATURAL_LAYERS: readonly string[] = Object.freeze([
  'equilibrium-functor','equilibrium-category','equilibrium-manifold','equilibrium-tensor','equilibrium-mechanics',
...
export function adaptAllNaturalInputs(snapshots: readonly PriorNaturalSnapshot[]): readonly NaturalComponent[] {
  return Object.freeze(snapshots.map(adapt));
}

export function buildDefaultNaturalInputs(): readonly NaturalComponent[] {
  const layers = CANONICAL_NATURAL_LAYERS;
