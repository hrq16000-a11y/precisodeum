...
// Allowlist explícita para cards de BUSCA. Removidas colunas pesadas/JSONB
// (`working_hours_struct`, `opens_*`, `is_24h`, `accepts_on_demand`) que só são
// necessárias na página de perfil individual (ProviderProfile usa um select
// próprio com PROVIDER_PUBLIC_COLS). `working_hours` (string curta) é mantido
// pois aparece em alguns cards/snippets.
const providerSelect = 'id, user_id, created_at, business_name, description, photo_url, city, state, neighborhood, latitude, longitude, phone, whatsapp, years_experience, slug, featured, rating_avg, review_count, status, category_id, portfolio_photo_count, portfolio_album_count, services_count, avg_response_minutes, community_verified, working_hours, categories(name, slug, icon)';
