...
const PROFILE_TYPES = [
  { value: 'client', label: 'Cliente', desc: 'Busco profissionais e serviços' },
  { value: 'provider', label: 'Profissional', desc: 'Ofereço serviços e quero clientes' },
  { value: 'rh', label: 'Agência / RH', desc: 'Publico vagas e oportunidades' },
];
          ...
          <div className="rounded-xl border border-border bg-card p-8 shadow-card">
            ...
            <div className="mt-6 flex rounded-lg bg-muted p-1">
              {PROFILE_TYPES.map(pt => (
                <button
                  key={pt.value}
                  className={`flex-1 rounded-md py-2 text-xs font-medium transition-colors ${accountType === pt.value ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground'}`}
                  onClick={() => setAccountType(pt.value)}
                >
                  {pt.label}
                </button>
              ))}
            ...
            <p className="mt-2 text-center text-xs text-muted-foreground">
              {PROFILE_TYPES.find(p => p.value === accountType)?.desc}
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Button type="submit" variant="accent" className="w-full" disabled={loading}>
                {loading ? 'Criando...' : `Criar Conta${accountType === 'provider' ? ' de Profissional' : accountType === 'rh' ? ' de RH' : ''}`}
              </Button>
            </form>
