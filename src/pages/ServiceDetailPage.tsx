...
const ServiceDetailPage = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: provider }, { data: profile }, { data: images }, { data: scats }] = await Promise.all([
        ...
        supabase.from('profiles').select('full_name, avatar_url').eq('id', (await supabase.from('providers').select('user_id').eq('id', svc.provider_id).maybeSingle()).data?.user_id || '').maybeSingle(),
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <a
        href={`https://wa.me/${svc.provider?.whatsapp || svc.whatsapp}`}
        target="_blank"
