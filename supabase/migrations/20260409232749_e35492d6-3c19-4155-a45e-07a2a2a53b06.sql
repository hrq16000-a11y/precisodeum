...
BEGIN
  ...
  -- Check if user has an active provider record
  SELECT EXISTS(
    SELECT 1 FROM providers WHERE user_id = v_user_id AND deleted_at IS NULL
  ) INTO v_has_provider;
  ...
  ELSIF v_service_count = 0 AND v_current_type = 'provider' AND NOT v_has_provider THEN
    -- Only revert to client if user has NO active provider record
