-- Estende o whitelist de métricas do log_web_vitals para aceitar os sinais de
-- saúde de imagem (hero/gallery) usados na correlação com picos de LCP.
...
BEGIN
  ...
  v_max := least(jsonb_array_length(_samples), 12);
  ...
  WHILE v_idx < v_max LOOP
    ...
    IF v_metric IS NULL OR v_metric NOT IN ('LCP','INP','CLS','FCP','TTFB','IMG_ERROR','IMG_DEGRADED') THEN
      CONTINUE;
    END IF;
